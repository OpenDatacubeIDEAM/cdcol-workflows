...
def perform_timeseries_analysis(dataset_in, no_data=-9999):
    ...
    clean_data_raw = np.reshape(np.in1d(data.values.reshape(-1), [no_data], invert=True), data.values.shape).astype(int)
    ...
    if hasattr(data, "latitude"):
        ...
        dataset_out = xr.Dataset(collections.OrderedDict(
            [('normalized_data', (['y', 'x'], processed_data_normalized.astype(np.float32))),
